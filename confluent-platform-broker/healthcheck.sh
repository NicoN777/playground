brokers_info=$(kafka-broker-api-versions --bootstrap-server kafka-broker-1:9092 --command-config ${KAFKA_CONF_DIR}/kafka-client.properties | awk '/id/{print $1}' | sort)
echo "${brokers_info}" > ${HOME}/kafka_brokers_api_versions.txt

function get_kafka_brokers() {
  broker_ids=$(zookeeper-shell znode1:2182 -zk-tls-config-file ${KAFKA_CONF_DIR}/zookeeper-client.properties <<EOF
ls /brokers/ids
quit
EOF
  )
...
function process_kafka_brokers() {
  ...
  for id in "${brokers[@]}"
  do
    temp=$(zookeeper-shell znode1:2182 -zk-tls-config-file ${KAFKA_CONF_DIR}/zookeeper-client.properties <<EOF
get /brokers/ids/${id}
quit
EOF
)
